...
PROTO_TABLE = {num: name[8:] for name, num in vars(
...
windows_mapping = {
    ...
    12: {
        ...
        "ProcessId": "process:pid",
        "ProcessGuid": "process:x_unique_id",
        # OR: "process:x_created_key_ref.key"?
        "TargetObject": "windows-registry-key:key",
    },
    13: {
        "UtcTime": ["first_observed", "last_observed"],
        "Image": split_image,
...
class SdsMapper(Mapper):
    ...
    def enhanced_action(message):
        ...
        if m:
            ...
            if event_id:
                results.append(
                    ('x-oca-event:action', windows_events.get(event_id) + ' - ' + event_type))
        m = re.search(r'Details: ([^"]*)', message)
        if m:
            details = m.group(1)
            if details.startswith('DWORD') or details.startswith('QWORD'):
                parts = details.split()
                results.append(('windows-registry-key:values',
    ...
    def process_answers(answers):
        ...
        for answer in answers:
            try:
                ...
                results.append(
                    (f"domain-name:resolves_to_refs[{i}].value", answer))
                i += 1
            except ValueError:
                pass
        return results
    ...
    def convert(self, event):
        ...
        if stream in self.zeek_mapping:
            mapping = merge_mappings(
                self.common_mapping, self.zeek_mapping[stream])
        else:
...
def read_csv(fp, mappers, ident):
    ...
    reader = csv.DictReader(
        fp, delimiter=sep, fieldnames=names, quoting=quoting)
    events = []
    for obj in reader:
